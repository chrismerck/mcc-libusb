...
start:
  ...
  while(1) {
	...
	free(sdataIn);
        break;
      case 'C':
      	printf("Testing USB-1608FSG Analog Input Scan in Continuous mode 8 channels\n");
	...
	fcntl(0, F_SETFL, flag | O_NONBLOCK);
	  ...
	  if (transferred != 256) {
	    fprintf(stderr, "test-usb1608G: number of bytes transferred = %d, \n", transferred);
	  } else {
	    printf("read 128 samples.\n");
	  }
          if (i%10 == 0) {
            printf("Scan = %d\n", i);
	  }
